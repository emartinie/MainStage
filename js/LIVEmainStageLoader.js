...
/ --- Render Bible Questions Card ---
async function renderQuestionsCard() {
  const container = document.getElementById("bibleStage");
  if (!container) return console.error("No bibleStage container found");

  // Clear inactive cards to keep performance smooth
  clearInactiveCards();
  ...
  try {
    ...
    // Create main card
    const questionsCard = createCard("Bible Questions", "");
    const content = questionsCard.querySelector(".collapse-content");
    ...
    questionEl.className = "text-lg font-semibold mb-2";
    answerEl.className = "text-gray-400 mb-4 hidden";
    nextBtn.className =
      "px-4 py-2 bg-indigo-600 text-white rounded-full shadow hover:bg-indigo-500 transition";

    function renderQuestion() {
      const q = questions[currentIndex];
      questionEl.textContent = Q: ${q.question};
      answerEl.textContent = A: ${q.answer};
    ...
    content.append(questionEl, answerEl, nextBtn);
    renderQuestion();
...
function clearInactiveCards() {
  const cardsContainer = document.getElementById("bibleStage");
  if (cardsContainer) cardsContainer.innerHTML = "";
}

if (currentMode === "questions") {
  await loadQuestionCards();
}
