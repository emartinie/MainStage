...
async function loadQuestionCards() {
  ...
  try {
    const res = await fetch('questions.json'); // Path to your JSON
    ...
    questions.forEach(q => {
      // Card wrapper
      const card = document.createElement('div');
      card.className = 'border rounded-lg p-4 shadow-lg bg-gray-800 hover:bg-gray-700 transition-colors duration-200';

      // Question
      const questionEl = document.createElement('h3');
      questionEl.textContent = q.question;
      questionEl.className = 'font-semibold text-lg mb-2 text-white';

      // Answer (hidden initially)
      const answerEl = document.createElement('div');
      answerEl.innerHTML = Answer: ${q.answer};
      answerEl.className = 'relative z-10 text-lg font-bold text-yellow-300 text-center mb-4 drop-shadow hidden';

      // Toggle button
      const toggleBtn = document.createElement('button');
      toggleBtn.textContent = 'Show Answer';
      toggleBtn.className = 'mt-2 px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors duration-200';
      toggleBtn.addEventListener('click', () => {
        answerEl.classList.toggle('hidden');
        toggleBtn.textContent = answerEl.classList.contains('hidden') ? 'Show Answer' : 'Hide Answer';
      });
      ...
      if (q.category) {
        ...
        tag.className = 'inline-block bg-yellow-500 text-black text-xs font-bold px-2 py-1 rounded mb-2';
    ...
    container.innerHTML = '<p class="text-red-500 font-bold">Failed to load questions.</p>';
  }
}
